...
const { config, oauthClient } = require('./lib/line');

const cookieSecret = functions.config().auth.cookiesecret;
const callbackUrl = functions.config().linelogin.callbackurl;
...
app.get('/auth/callback', asyncHandler(async (req, res) => {
  ...
  const decoded = jwt.verify(token.id_token, config.client.secret, { algorithms: ['HS256'] });
